...
class TouchView : AppCompatImageView {
  ...
  @SuppressLint("ClickableViewAccessibility") private fun sharedConstructing(context: Context) {
  ...
  fun setZoom(scale: Float, focusX: Float, focusY: Float) {
    mScaleType?.let { setZoom(scale, focusX, focusY, it) }
  }
   ...
   * @param
   */
  fun setZoom(img: TouchView) {
    val center = img.getScrollPosition()
    img.scaleType?.let { setZoom(img.getCurrentZoom(), center!!.x, center.y, it) }
  }
  ...
  override fun canScrollHorizontally(direction: Int): Boolean {
    ...
    return if (!isHorizontalScrollEnabled) {
      false
    } else ((!(getImageWidth() < viewWidth)) && (!(x >= -1) || direction >= 0) && (!(Math.abs(x) + viewWidth + 1 >= getImageWidth()) || direction <= 0))
  }
